...
export default function AuthForm() {
  ...
  const { signUp, signIn, signInWithGoogle, error, user, emailVerificationSent, resendVerificationEmail } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Usually wait for email verification, but if you allow immediate access:
        // router.push("/dashboard") 
      } else {
        await signIn(email, password)
        router.push("/dashboard") // ✅ Redirect after email login
      }
    } catch (err) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      router.push("/dashboard") // ✅ Redirect immediately after Google success
    } catch (err: any) {
      console.error("Google sign-in error:", err)
    } finally {
      // We don't set loading to false here if successful, 
      // because we want the spinner to stay while redirecting.
      // We only stop loading if there was an error.
      // (However, to be safe in case redirect is slow, we can check user state)
    }
  }
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
              ...
              <p className="text-xs text-blue-600 mt-2">
                We sent a link to <strong>{email}</strong>. Click it to verify your account.
              </p>
            </div>
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="grid md:grid-cols-2 gap-8 max-w-6xl w-full">
        ...
        <div className="flex items-center justify-center">
          <Card className="w-full max-w-sm border-none shadow-none md:border md:shadow-sm">
            <CardHeader>
